"""
Code inspired from the official Github repo for '3D Hand Shape and Pose from Images in the Wild'  https://github.com/boukhayma/3dhand
"""
...
class _ResNet_Mano(nn.Module):
    def __init__(self, pretrain, block, layers, backend, input_option, num_classes=1000):
        self.pretrain = pretrain
        self.input_option = input_option
        self.inplanes = 64
        super(_ResNet_Mano, self).__init__()

        #cfg_backmodel = edict( {"name":'resnet50'} )
        self.backend_model = model_factory.get_model(backend)
    ...
    def forward(self, x):
        ...
        # if self.pretrain:
        #     return xs
        xs = self.backend_model(x)["pca_hand_pose"]
...
def getBou50(ispretrain=False, backend = None, input_option=1, **kwargs):
    
    model = _ResNet_Mano(ispretrain, BasicBlock, [3, 4, 6, 3], backend, input_option, **kwargs)
